...
//		view
		...
		private void onContactoChanged(ObservableValue<? extends Contacto> o, Contacto ov, Contacto nv) {
			...
			//bind tabla telefonos
				telefonosTable.itemsProperty().bindBidirectional(ov.telefonosProperty());
			//bind tabla email
				emailTable.itemsProperty().bindBidirectional(ov.emailsProperty());
			//bind tabla web
				webTable.itemsProperty().bindBidirectional(ov.websProperty());
			}
		}
	    ...
	    void onAnadirTelefonoAction(ActionEvent event) {
	    	...
	    	if (result.isPresent()){
	    	    contacto.getValue().getTelefonos().add(result.get());
	    	}
	    }
